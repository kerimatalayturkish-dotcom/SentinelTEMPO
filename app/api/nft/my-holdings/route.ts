...
// Reads use the private RPC (Alchemy) for unthrottled ownerOf/tokenURI calls.
const reads = createPublicClient({ chain: tempoChain, transport: serverHttp() })

// Logs use the public Tempo RPC — the private endpoint can be picky about
// large indexed-arg getLogs ranges. Public RPC handles small chunked queries
// reliably.
...
const LOOKBACK_BLOCKS = 3_000_000n   // ~17 days at 0.5s blocks; covers full launch
  ...
  try {
    ...
    const enriched = await Promise.all(stillHeld.map(async ({ tokenId }) => {
      ...
      return {
        ...
        tokenURI,
        image,
        name: name ?? `SentinelTEMPO #${tokenId}`,
        attributes,
      }
    }))
